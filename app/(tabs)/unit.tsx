...
export default function Unit() {
  ...
  const deleteUnit = async (unitId: string) => {
    setIsLoading(true);
    try {
      const response = await deleteUnitApi(unitId, user?.token as string);
      if (response?.status === 200) {
  ...
  return (
    <ScrollView>
      <View style={styles.container}>
        ...
        <TouchableOpacity
          onPress={() => router.push("/(admin)/unit-form")}
          style={styles.btn}
        >
          <Text style={{ color: "#fff" }}>New Unit</Text>
        </TouchableOpacity>
      </View>
      {units.length ? (
        units.map((unit) => (
          <CategoryItem
            key={unit._id}
            name={unit.name}
            disabled={!unit.visibility}
            editCategory={() =>
              router.push({
                pathname: "/(admin)/unit-form",
                params: { id: unit._id, name: unit.name },
              })
            }
            deleteCategory={() => deleteUnit(unit._id)}
          />
        ))
      ) : (
        <View style={styles.emptyContainer}>
          <Text style={styles.emptyUnits}>No units available.</Text>
        </View>
      )}
    </ScrollView>
  );
